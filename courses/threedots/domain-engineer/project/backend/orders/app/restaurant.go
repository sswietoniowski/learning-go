...
func (s *Service) MarkOrderAsReadyForPickup(ctx context.Context, restaurantUUID RestaurantUUID, orderUUID OrderUUID) error {
	return s.orderRepository.UpdateOrder(
		...
		func(ctx context.Context, order Order) (Order, error) {
			...
			if order.RestaurantPreparedAt != nil {
				// Idempotent: the first prepared timestamp matters, we don't want to overwrite it.
				log.FromContext(ctx).With("order_uuid", orderUUID).Warn("Order already marked as ready for pickup")
				return order, nil
			}
