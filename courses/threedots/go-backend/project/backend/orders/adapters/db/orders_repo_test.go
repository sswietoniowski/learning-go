...
func TestPlaceOrder(t *testing.T) {
	...
	fetchedQuote, err := ordersRepo.GetQuote(ctx, quote.QuoteUUID)
	require.NoError(t, err)
	menuItems, err := ordersRepo.GetMenuItemsForQuote(ctx, quote.QuoteUUID, quote.RestaurantUUID)
	require.NoError(t, err)
...
func TestUpdateOrder(t *testing.T) {
	...
	err = ordersRepo.UpdateOrder(ctx, originalOrder.OrderUUID, func(ctx context.Context, order app.Order) (app.Order, error) {
		...
		expectedOrder = app.Order{
			...
			RestaurantConfirmedAt: &confirmedAt,
			CourierAcceptedAt:     &acceptedAt,
			RestaurantPreparedAt:  &preparedAt,
