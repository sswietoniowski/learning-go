...
func (q *Queries) GetRestaurantName(ctx context.Context, restaurantUuid app.RestaurantUUID) (string, error) {
	row := q.db.QueryRow(ctx, getRestaurantName, restaurantUuid)
	var name string
	err := row.Scan(&name)
	return name, err
}
